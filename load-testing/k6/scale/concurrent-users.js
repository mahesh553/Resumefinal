...
function browseJobApplications(token) {
  ...
  const response = http.get(
    `${BASE_URL}/api/job-applications?page=1&limit=50`,
    {
      headers,
      timeout: "10s",
    }
...
function loadAdminDashboard(token) {
  ...
  return check(response, {
    ...
    "admin dashboard response time acceptable": (r) =>
...
export function handleSummary(data) {
  const report = {
    ...
    scale_analysis: {
      max_concurrent_users: Math.max(
        ...Object.keys(data.metrics.vus || {}).map(Number)
      ),
      peak_load_success_rate: (
        (data.metrics.peak_load_handling?.values?.rate || 0) * 100
      ).toFixed(2),
      overall_success_rate: (
        (data.metrics.concurrent_user_success_rate?.values?.rate || 0) * 100
      ).toFixed(2),
      system_stability_rate: (
        (data.metrics.system_stability_rate?.values?.rate || 0) * 100
      ).toFixed(2),
      resource_exhaustion_events:
        data.metrics.resource_exhaustion_events?.count || 0,
      avg_response_time_under_load: (
        data.metrics.response_time_under_load?.values?.avg || 0
      ).toFixed(2),
      p95_response_time_under_load: (
        data.metrics.response_time_under_load?.values?.["p(95)"] || 0
      ).toFixed(2),
    },
    performance_verdict: {
      can_handle_peak_load:
        data.metrics.peak_load_handling?.values?.rate > 0.85 ? "PASS" : "FAIL",
      system_stability:
        data.metrics.system_stability_rate?.values?.rate > 0.95
          ? "PASS"
          : "FAIL",
      response_time_acceptable:
        (data.metrics.response_time_under_load?.values?.["p(95)"] || 0) < 5000
          ? "PASS"
          : "FAIL",
      ...
      no_resource_exhaustion:
        (data.metrics.resource_exhaustion_events?.count || 0) < 10
          ? "PASS"
          : "FAIL",
    },
  };
  ...
  return {
    "load-testing/reports/results/concurrent-users-scale-test.json":
