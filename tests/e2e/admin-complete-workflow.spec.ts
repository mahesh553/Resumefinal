...
test.describe("Complete Admin Workflow E2E Tests", () => {
  ...
  test("should complete full admin dashboard workflow", async ({ page }) => {
    ...
    await expect(
      page.locator(`text=e2etest${timestamp}@example.com`)
    ).toBeVisible();
  ...
  test("should perform bulk user operations", async ({ page }) => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      await page.click('button:has-text("Create User")');
      await expect(
        page.locator("text=User created successfully")
      ).toBeVisible();
    }
    ...
    if (await bulkActionsButton.isVisible()) {
      ...
      if (await deleteOption.isVisible()) {
        ...
        await page.click('button:has-text("Delete")');
        await expect(
          page.locator("text=Users deleted successfully")
        ).toBeVisible();
      }
    } else {
  ...
  test("should test real-time updates and polling", async ({ page }) => {
    ...
    // Get initial values
    const initialUptime = await page
      .locator('[data-testid="system-uptime"]')
      .textContent();
  ...
  test("should handle error recovery and retry mechanisms", async ({
    ...
    for (const testPage of testPages) {
      ...
      const errorMessage = page.locator(
        "text=Failed to load, text=Error loading, text=Unable to fetch"
      );
  ...
  test("should maintain consistent UI state across browser refresh", async ({
    page,
  }) => {
  ...
  test("should validate data consistency across admin sections", async ({
    page,
  }) => {
    ...
    const dashboardUserCount = await page
    ...
    if (await paginationInfo.isVisible()) {
      ...
      if (totalMatch) {
        ...
        expect(Math.abs(dashboardCount - managementCount)).toBeLessThanOrEqual(
          5
        );
      }
    }
