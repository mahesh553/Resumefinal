...
test.describe('System Monitoring E2E Tests', () => {
  ...
  test('should display overall system health status', async ({ page }) => {
    ...
    expect(statusText?.toLowerCase()).toMatch(/healthy|warning|critical|degraded/);
  ...
  test('should display service health cards', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="service-health-cards"]', { timeout: 15000 });
    
    // Check individual service cards
    await expect(page.locator('[data-testid="database-health-card"]')).toBeVisible();
    await expect(page.locator('[data-testid="redis-health-card"]')).toBeVisible();
    await expect(page.locator('[data-testid="ai-services-health-card"]')).toBeVisible();
  ...
  test('should display network and API metrics', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="network-metrics"]', { timeout: 10000 });
    
    // Check active connections
    const activeConnections = page.locator('[data-testid="active-connections"]');
  ...
  test('should display real-time charts', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="monitoring-charts"]', { timeout: 15000 });
    ...
    const responseTimeChart = page.locator('[data-testid="response-time-chart"]');
  ...
  test('should handle service detail views', async ({ page }) => {
    ...
    if (await databaseCard.isVisible()) {
      ...
      const detailSection = page.locator('[data-testid="service-detail-section"]');
  ...
  test('should display queue and background job status', async ({ page }) => {
    ...
    if (await queueSection.isVisible()) {
      ...
      const completedJobs = page.locator('[data-testid="completed-jobs-count"]');
  ...
  test('should handle API monitoring failures', async ({ page }) => {
    ...
    const fallbackMessage = page.locator('text=Unable to connect to monitoring service');
  ...
  test('should display historical data views', async ({ page }) => {
    ...
    const timeRangeSelector = page.locator('[data-testid="monitoring-time-range"]');
  ...
  test('should be responsive on mobile devices', async ({ page, isMobile }) => {
    ...
    // Check mobile layout
    const healthOverview = page.locator('[data-testid="system-health-overview"]');
