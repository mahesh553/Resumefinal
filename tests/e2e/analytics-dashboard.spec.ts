...
test.describe('Analytics Dashboard E2E Tests', () => {
  ...
  test('should display analytics dashboard interface', async ({ page }) => {
    ...
    await expect(page.locator('select[aria-label*="Time range"]')).toBeVisible();
  ...
  test('should display key metrics cards', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="resume-uploads-metric"]')).toBeVisible();
    await expect(page.locator('[data-testid="success-rate-metric"]')).toBeVisible();
    
    // Verify metrics have values
    const metrics = page.locator('[data-testid$="-metric"] .text-2xl, [data-testid$="-metric"] .text-3xl');
  ...
  test('should display user growth chart', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="user-growth-chart"]', { timeout: 15000 });
    
    // Chart should be visible
    await expect(page.locator('[data-testid="user-growth-chart"]')).toBeVisible();
    
    // Should contain SVG (Recharts renders as SVG)
    await expect(page.locator('[data-testid="user-growth-chart"] svg')).toBeVisible();
  ...
  test('should display resume activity chart', async ({ page }) => {
    // Wait for charts to load
    await page.waitForSelector('[data-testid="resume-activity-chart"]', { timeout: 15000 });
    
    // Chart should be visible
    await expect(page.locator('[data-testid="resume-activity-chart"]')).toBeVisible();
    
    // Should contain SVG
    await expect(page.locator('[data-testid="resume-activity-chart"] svg')).toBeVisible();
  ...
  test('should display popular features section', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="popular-features"]', { timeout: 10000 });
  ...
  test('should change time range and update data', async ({ page }) => {
    ...
    const initialUserCount = await page.locator('[data-testid="total-signups-metric"] .text-3xl').textContent();
