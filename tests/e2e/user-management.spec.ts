...
test.describe("User Management E2E Tests", () => {
  ...
  test("should display user management interface", async ({ page }) => {
    ...
    await expect(
      page.locator('select[aria-label*="Filter by role"]')
    ).toBeVisible();
    await expect(
      page.locator('select[aria-label*="Filter by status"]')
    ).toBeVisible();
  });
  ...
  test("should filter users by role", async ({ page }) => {
    ...
    const roleBadges = page.locator(
      "tbody tr .bg-purple-100, tbody tr .bg-red-100"
    );
  ...
  test("should open create user modal", async ({ page }) => {
    ...
    await expect(
      page.locator('input[type="checkbox"][name="sendWelcomeEmail"]')
    ).toBeVisible();
  ...
  test("should confirm user deletion", async ({ page }) => {
    ...
    for (let i = 0; i < rowCount; i++) {
      ...
      if (roleText?.toLowerCase() !== "admin") {
        ...
        await expect(
          page.locator("text=Are you sure you want to delete this user?")
        ).toBeVisible();
    ...
    for (let i = 0; i < rowCount; i++) {
      ...
      if (roleText?.toLowerCase() === "user") {
        ...
        if (await promoteButton.isVisible()) {
          ...
          // Should show success message
          await expect(
            page.locator("text=User promoted successfully")
          ).toBeVisible();
    ...
    for (let i = 0; i < rowCount; i++) {
      ...
      if (statusText?.toLowerCase() === "active") {
        ...
        if (await suspendButton.isVisible()) {
          ...
          // Should show success message
          await expect(
            page.locator("text=User suspended successfully")
          ).toBeVisible();
