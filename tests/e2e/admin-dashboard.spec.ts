...
test.describe('Admin Dashboard E2E Tests', () => {
  ...
  test('should display admin dashboard overview', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="total-resumes-metric"]')).toBeVisible();
    ...
    const totalUsers = page.locator('[data-testid="total-users-metric"] .text-3xl');
    await expect(totalUsers).not.toBeEmpty();
    
    const totalResumes = page.locator('[data-testid="total-resumes-metric"] .text-3xl');
  ...
  test('should display system status indicators', async ({ page }) => {
    ...
    if (await statusSection.isVisible()) {
      ...
      await expect(statusSection.locator('[data-testid="database-status"]')).toBeVisible();
      await expect(statusSection.locator('[data-testid="redis-status"]')).toBeVisible();
      await expect(statusSection.locator('[data-testid="ai-status"]')).toBeVisible();
    }
  });
  ...
  test('should handle concurrent data updates', async ({ page }) => {
    ...
    // Get initial metrics
    const initialUserCount = await page.locator('[data-testid="total-users-metric"] .text-3xl').textContent();
