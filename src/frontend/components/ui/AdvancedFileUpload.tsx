...
export function AdvancedFileUpload({
      ...
      rejectedFiles.forEach((file) => {
        ...
        errors.forEach((error: any) => {
          if (error.code === 'file-too-large') {
            toast.error(`File ${file.file.name} is too large. Max size: ${Math.round(maxSize / 1024 / 1024)}MB`);
          } else if (error.code === 'file-invalid-type') {
            toast.error(`File ${file.file.name} is not supported. Accepted: ${acceptedTypes.join(', ')}`);
          } else {
      ...
      prev.map((f) =>
        f.id === file.id
          ? {
              ...
              analysis: Math.random() > 0.1 ? {
                atsScore: Math.floor(Math.random() * 40) + 60,
                skillsFound: Math.floor(Math.random() * 10) + 5,
                improvements: Math.floor(Math.random() * 5) + 2,
              } : null,
            }
          : f
      )
  ...
  const getFileIcon = (file: File) => {
    ...
    if (file.type.includes('word') || file.name.endsWith('.docx') || file.name.endsWith('.doc')) return FiFile;
      ...
      {files.length > 0 && (
        ...
        >
          ...
          <Button
            ...
            className="ml-auto"
          >
            Analyze Resumes ({files.filter(f => f.status === 'success').length})
          </Button>
        </motion.div>
      )}
    </div>
