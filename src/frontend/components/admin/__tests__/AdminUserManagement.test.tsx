...
describe('AdminUserManagement Component', () => {
  ...
  describe('Initial Render', () => {
    beforeEach(() => {
      mockUseQuery
        .mockReturnValueOnce({
          data: { users: mockUsers, pagination: { total: 3, page: 1, limit: 10 } },
          isLoading: false,
          error: null,
        } as any)
        .mockReturnValueOnce({
    ...
    it('should change items per page', async () => {
      ...
      await waitFor(() => {
        expect(mockUseQuery).toHaveBeenCalledWith(
          ...
          expect.any(Function),
          expect.objectContaining({
            queryKey: expect.arrayContaining([expect.objectContaining({ limit: 25 })])
          })
        );
      });
    });
  ...
  describe('Error Handling', () => {
    ...
    it('should display mutation errors', async () => {
      const mockMutationWithError = jest.fn().mockRejectedValue(
  ...
  describe('Bulk Actions', () => {
    ...
    it('should allow selecting multiple users', () => {
      ...
      expect(userCheckboxes.filter(cb => cb.checked).length).toBe(userCheckboxes.length);
    });
